...
<template>
  <div>
    ...
    <b-progress v-if="taskCounters.total_sessions" :value="taskCounters.tasks['DONE'] + taskCounters.tasks['ERROR']" size="is-medium" :max="taskCounters.total_tasks" type="is-info" show-value>
    ...
    <span v-for="sessionId in sessionIds" :key="sessionId">
      <ts-analysis-session-detail :session-id="sessionId" @closeDetail="sessionIds.splice(sessionIds.indexOf(sessionId),1)"></ts-analysis-session-detail>
    </span>
