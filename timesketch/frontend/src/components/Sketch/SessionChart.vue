            ...
            <div class="control">
                <div class="select">
                    <select v-model="selectedType" @change="selectSessionType">
                        ...
                        <option v-for="sessionType
                         in sessionTypes" :value=sessionType
                         v-bind:key="sessionType
                        ">{{sessionType
                            }}</option>
                    </select>
                </div>
            ...
            <div class="control">
                <div class="select">
                    <select v-model="selectedID" @change="selectSessionID">
                        <option disabled value="">Please select one</option>
                        <option v-for="session in selectedSessions" :value=session.sessionId v-bind:key="session.sessionType
                         + session.sessionId">{{session.sessionId}}</option>
                    </select>
                </div>
            </div>
        </div>
  ...
  methods: {
    registerClickListener: function (view) {
      ...
      view.addEventListener('click', function (event, item) {
        var sessionType =
         item.datum.sessionType

        var sessionId = item.datum.sessionId
        if (sessionType !==
         undefined && sessionId !== undefined) {
          var queryString = 'sessionId.' + sessionType +
           ':' + sessionId
      ...
      if (this.showChart) {
        ...
        if (this.sessions.length === 0) {
          ApiClient.getSessions(this.sketch.id, this.indices[0]).then((response) => {
            ...
            this.sessionTypes = this.sessionTypes.concat([...new Set(this.sessions.map(s => s.sessionType
            ))])
            this.getVegaSpec()
          }).catch((e) => {})
        } else {
    ...
    selectSessionType: function (event) {
      ...
      if (this.selectedType === 'all') {
        ...
        this.selectedSessions = this.processedSessions.filter(session => session['sessionType'] === this.selectedType)
    ...
    selectSessionID: function (event) {
      ...
      for (var i = 0; i < selectedSessionsCopy.length; i++) {
        ...
        if (this.selectedType === session.sessionType &&
         this.selectedID === session.sessionId) {
          session['selected'] = true
          queryString = 'sessionId.' + session['sessionType'] + ':' + session['sessionId']
    ...
    getProcessedSessions: function () {
      ...
      if (processedSessions.length > 0) {
        var largestTimestamp = processedSessions[0].end_timestamp
        ...
        if (this.endTimeRange == null) {
          for (var i = 1; i < processedSessions.length; i++) {
            if (processedSessions[i].end_timestamp > largestTimestamp) {
              largestTimestamp = processedSessions[i].end_timestamp
            }
          }
          this.endTimeRange = largestTimestamp
          ...
          smallestTimestamp = processedSessions[0].start_timestamp
          ...
          smallestTimestamp = processedSessions[0].start_timestamp
          largestTimestamp = processedSessions[0].end_timestamp
          // eslint-disable-next-line no-redeclare
          for (var i = 1; i < processedSessions.length; i++) {
            if (processedSessions[i].start_timestamp < smallestTimestamp) {
              smallestTimestamp = processedSessions[i].start_timestamp
        ...
        var timeRange = (largestTimestamp - smallestTimestamp)
        ...
        for (var i = 0; i < processedSessions.length; i++) {
          ...
          var extentedEnd = session.start_timestamp + this.barSize
          if (session.end_timestamp < extentedEnd) {
            session.end_timestamp = extentedEnd
          }
        }
      }
    ...
    getVegaSpec: function () {
      ...
      var dictSpec = { '$schema': 'https://vega.github.io/schema/vega-lite/v3.json',
        ...
        'vconcat': [{
          ...
          'encoding': {
            'y': { 'field': 'sessionType', 'type': 'ordinal', 'axis': { 'title': 'Session Type' } },
