...
class BaseAnalyzer:
    ...
    def __init__(self, index_name, sketch_id, timeline_id=None):
        ...
        # Add AnalyzerOutput instance and set all attributes that can be set automatically
...
class AnalyzerOutput:
    ...
    Attributes:
        ...
        saved_views (List[int]): [Optional] Saved views generatred by the analyzer.
        saved_stories (List[int]): [Optional] Saved stories generated by the analyzer.
        saved_graphs (List[int]): [Optional] Saved graphs generated by the analyzer.
        saved_aggregations (List[int]): [Optional] Saved aggregations generated by the analyzer.
    ...
    def validate(self):
        ...
        schema = {
            ...
            "properties": {
                ...
                "saved_stories": {"type": "array", "items": [{"type": "integer"}]},
                "saved_aggregations": {
                    "type": "array",
                    "items": [
                        {"type": "integer"},
        ...
        try:
            ...
            raise AnalyzerOutputException(f"json schema error: {e}")
    ...
    def to_json(self):
        ...
        output = {
            ...
            "timesketch_instance": self.timesketch_instance,
        ...
        if self.references and self.references != []:
            output["references"] = self.references

        if self.attributes and self.attributes != {}:
            output["attributes"] = self.attributes

        if self.saved_views and self.saved_views != []:
            output["saved_views"] = self.saved_views

        if self.saved_stories and self.saved_stories != []:
            output["saved_stories"] = self.saved_stories

        if self.saved_graphs and self.saved_graphs != []:
            output["saved_graphs"] = self.saved_graphs

        if self.saved_aggregations and self.saved_aggregations != []:
            output["saved_aggregations"] = self.saved_aggregations

        if self.created_tags and self.created_tags != []:
            output["created_tags"] = self.created_tags
