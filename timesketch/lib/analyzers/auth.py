...
class AuthAnalyzer:
    ...
    def session_duration(self, session_id: str, timestamp: int) -> int:
        ...
        try:
            ...
            log.error(
                "[%s] Error getting session start time for %s. %s",
                self.NAME,
                session_id,
                str(e),
            )
        ...
        try:
            ...
            log.error(
                "[%s] Error getting session end time for %s. %s",
                self.NAME,
                session_id,
                str(e),
            )
    ...
    def get_auth_summary(
        self, df: pd.DataFrame, summary_type: str, value: str
    ) -> AuthSummaryData:
        ...
        if success_df.empty:
            log.info(
                "[%s] No successful events for %s: %s", self.NAME, summary_type, value
            )
            return summary
        ...
        summary.success_source_ip_list = list(set(success_df["source_ip"].to_list()))
        ...
        summary.distinct_source_ip_count = len(auth_summary_df["source_ip"].unique())
        ...
        summary.top_source_ips = (
            auth_summary_df.groupby(by="source_ip")["timestamp"]
            .nunique()
            .nlargest(10)
            .to_dict()
        )
        summary.top_usernames = (
            auth_summary_df.groupby(by="username")["timestamp"]
            .nunique()
            .nlargest(10)
            .to_dict()
        )
    ...
    def get_login_session(
        ...
        if login_df.empty:
            log.debug(
                "[%s] No successful login for IP %s and session ID %s",
                NAME,
                source_ip,
                session_id,
            )
        ...
        try:
            ...
            log.error(
                "[%s] Error getting login timestamp for session ID %s. %s",
                NAME,
                session_id,
                str(exception),
            )
        ...
        login_session = LoginRecord(
            source_ip=source_ip, domain=domain, username=username, session_id=session_id
        )
        ...
        log.debug(
            ...
            login_session.session_duration,
        )
...
class BruteForceAnalyzer(AuthAnalyzer):
    ...
    def login_analysis(self, source_ip: str) -> AuthSummaryData:
        ...
        for _, row in success_df.iterrows():
            ...
            bf_stat_df = bruteforce_window_df.groupby(by="auth_result")[
                "timestamp"
            ].count()
            ...
            try:
                ...
                #  dataframe.
                log.info(
                    "[%s] No successful login events from source IP %s",
                    self.NAME,
                    source_ip,
                )
    ...
    def generate_analyzer_output(
        self, summaries: List[AuthSummaryData], analyzer_success: bool
    ) -> AnalyzerOutput:
