...
class StatusRaceTest(BaseTest):
    ...
    def test_reproduce_race_condition(self):
        """Reproduce the race condition leading to duplicate statuses."""
        ...
        # 2. Simulate two workers using DIFFERENT sessions.
        session1 = db_session
        ...
        # Now both workers commit (though set_status already commits)
        session1.commit()
        session2.commit()
        session2.close()

        # 3. Verify the fix: exactly one status should remain, and it should be 'status_B'
        session1.expire_all()
