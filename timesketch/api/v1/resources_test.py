...
class ViewListResourceTest(BaseTest):
    ...
    def test_post_view_list_resource(self):
        ...
        self.login()
        data = dict(
            name="test", new_searchtemplate=False, query="test", filter={}, dsl={}
        )
        response = self.client.post(
            self.resource_url,
            data=json.dumps(data, ensure_ascii=False),
...
class EventAnnotationResourceTest(BaseTest):
    ...
    def test_post_annotate_resource(self):
        ...
        for annotation_type in ["comment", "label"]:
            ...
            data = dict(
                annotation="test", annotation_type=annotation_type, events=[event]
            )
            response = self.client.post(
                self.resource_url,
    ...
    def test_post_annotate_invalid_index_resource(self):
        ...
        response = self.client.post(
            self.resource_url, data=json.dumps(data), content_type="application/json"
        )
