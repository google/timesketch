...
class SketchResourceTest(BaseTest):
    ...
    def test_archive_sketch_with_failed_timeline(self):
        """Authenticated request to archive a sketch with a failed timeline."""
        self.login()

        # 1. Create a new sketch
        sketch_name = "test_archive_with_failed_timeline"
        data = {"name": sketch_name, "description": "a sketch"}
        response = self.client.post(
            "/api/v1/sketches/",
            data=json.dumps(data),
            content_type="application/json",
        ...
        # 2. Create SearchIndex objects
        searchindex1 = SearchIndex.get_or_create(
            name="si1", description="si1", user=self.user1, index_name="si1_es"
        )
        searchindex2 = SearchIndex.get_or_create(
            name="si2", description="si2", user=self.user1, index_name="si2_es"
        )
        self._commit_to_database(searchindex1)
        self._commit_to_database(searchindex2)

        # 3. Add a 'ready' timeline
        timeline_url = f"/api/v1/sketches/{sketch_id}/timelines/"
        data = {"timeline": searchindex1.id}
        response = self.client.post(
            timeline_url,
            data=json.dumps(data),
            content_type="application/json",
        ...
        # 4. Add a second timeline that we will set to 'fail'
        data = {"timeline": searchindex2.id}
        response = self.client.post(
            timeline_url,
            data=json.dumps(data),
            content_type="application/json",
        ...
        self.assertEqual(HTTP_STATUS_CODE_CREATED, response.status_code)
        failed_timeline_id = response.json["objects"][0]["id"]
        failed_timeline_name = response.json["objects"][0]["name"]

        # 5. Set the status of the second timeline to 'fail'
        timeline_to_fail = Timeline.get_by_id(failed_timeline_id)
        timeline_to_fail.set_status("fail")
        self._commit_to_database(timeline_to_fail)

        # 6. Attempt to archive the sketch
        archive_url = f"/api/v1/sketches/{sketch_id}/archive/"
        data = {"action": "archive"}
        response = self.client.post(
            archive_url,
            data=json.dumps(data),
        ...
        # 7. Assert that the archival failed with the correct error
        self.assertEqual(HTTP_STATUS_CODE_BAD_REQUEST, response.status_code)
        expected_error_msg = (
            f"Cannot archive sketch {sketch_id}. Timeline "
            f"'{failed_timeline_name}' (ID: {failed_timeline_id}) is in "
            "a non-archivable state: 'fail'. Please delete this timeline "
            "and try again."
        )
        self.assertIn(expected_error_msg, response.json["message"])
