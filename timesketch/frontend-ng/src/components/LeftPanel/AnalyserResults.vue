  ...
  <div>
    ...
    >
      ...
      <span class="float-right mr-2">
        <v-progress-circular
          v-if="!analyzerResultsReady || (activeAnalyzerSessions.pending + activeAnalyzerSessions.started) > 0"
          :size="20"
          :width="1"
          indeterminate
          color="primary"
        >
          <small v-if="(activeAnalyzerSessions.pending + activeAnalyzerSessions.started) > 0">
            {{ (activeAnalyzerSessions.pending + activeAnalyzerSessions.started) }}
          </small>
        </v-progress-circular>
    ...
    <v-expand-transition>
      <div v-show="expanded">
        <div v-if="totalNumberOfAnalyzerResults > 0">
          <!-- TODO: issue#2565 -->
    ...
    fetchActiveAnalyzerSessions() {
      ApiClient.getActiveAnalyzerSessions(this.sketch.id)
        .then(response => {
          ...
          this.activeAnalyzerSessions.pending = taskCounters.tasks_status_count.PENDING
          this.activeAnalyzerSessions.started = taskCounters.tasks_status_count.STARTED
        })
        .catch(e => {
          console.error(e)
        })
