        ...
        <v-row no-gutters class="mt-5">
          <v-dialog width="500">
            <template v-slot:activator="{ on, attrs }">
              <v-btn depressed small class="mr-5" color="primary" v-bind="attrs"
                v-on="on"> Blank sketch </v-btn>
            </template>
            <v-card class="pa-3">
              <h3>New sketch</h3>
              <br />
              <v-text-field v-model="sketchForm.name" outlined dense
                placeholder="Name your sketch"> </v-text-field>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :disabled="!sketchForm.name" @click="createSketch()"
                  color="primary" text> Create </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
          <v-divider vertical class="mr-5"></v-divider>
          <v-btn v-for="scenario in scenarioTemplates"
            :key="scenario.short_name" depressed small outlined class="mr-5">
