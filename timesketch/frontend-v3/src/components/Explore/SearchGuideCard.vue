...
-->
<template>
  <v-card :variant="!flat ? 'outlined' : 'flat'" :class="{ 'ma-4': !flat }">
    <!-- Header Section -->
    ...
    <v-card-text>
      <v-row no-gutters>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a
                      href="#"
                      @click.prevent="emitSetQueryAndFilter('event_identifier:(4624 OR 4625) AND NOT LogonType:3')"
                    >
        ...
        <v-col v-if="showDataTypes">
          <div class="d-flex" style="height: 100%">
          <v-divider vertical></v-divider>
            <div class="pa-4 flex-grow-1">
              <h5><v-icon start>mdi-database-outline</v-icon> Data Types</h5>
              <DataTypesList @search-triggered="$emit('search-triggered')"></DataTypesList>
            </div>
          </div>
        </v-col>
        <v-col v-if="showSavedSearches">
          <div class="d-flex" style="height: 100%">
          <v-divider vertical></v-divider>
            <div class="pa-4 flex-grow-1">
              <h5><v-icon start>mdi-content-save-outline</v-icon> Saved Searches</h5>
              <SavedSearchesList @search-triggered="$emit('search-triggered')"></SavedSearchesList>
            </div>
          </div>
        </v-col>
      </v-row>
    ...
    <v-divider></v-divider>
    <v-card-actions>
      <v-btn variant="text" color="primary" href="https://timesketch.org/guides/user/search-query-guide/" target="_blank">
        Full Search Guide
